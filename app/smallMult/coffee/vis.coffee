...
SmallMults = () ->
  ...
  colorScale = d3.scale.ordinal()
    .range(["#5777F2", "#5777F2", "#5777F2", "#5777F2", "#5777F2", "#5777F2", "#5777F2", "#5777F2","#5777F2", "#5777F2", "#5777F2", "#5777F2"])

  # names will also be used to color the bars
  colorScale2 = d3.scale.ordinal()
    .range(["#E04F4C", "#E04F4C", "#E04F4C", "#E04F4C", "#E04F4C", "#E04F4C", "#E04F4C", "#E04F4C","#E04F4C", "#E04F4C", "#E04F4C", "#E04F4C"])
...
#      .attr("x", (d) -> xScale(d.name))
#      .attr("y", (d) -> (graphHeight - yScale(d.value+d.value2) - yPadding))
#      .attr("width", xScale.rangeBand())
#      .attr("height", (d) ->  yScale(d.value))
#      .attr("fill", (d) -> colorScale2(d.name))
    ...
    graph2 = base.append("g")
    graph2.selectAll(".bar")
      .data((d) -> d.values)
      .enter().append("rect")
      .attr("x", (d) -> xScale(d.name))
      .attr("y", (d) -> (graphHeight - yScale(d.value)- yScale(d.value2)- yPadding))
      .attr("width", xScale.rangeBand())
      .attr("height", (d) ->  yScale(d.value2))
      .attr("fill", (d) -> colorScale2(d.name))
      .on("mouseover", showAnnotation)
      .on("mouseout", hideAnnotation)
  ...
  setScales = () ->
    ...
    yScale.domain([0,yMax + 40000]) #yMax + 500000
